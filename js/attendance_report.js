...
export async function displayAttendanceReport() {
    try {
        ...
        const reportData = allData.find(item => item && item.action === 'attendanceReport');
        ...
        const { start_date, end_date, overall_stats, participant_data } = reportData.data;

        if (!start_date || !end_date || !overall_stats || !participant_data) {
            document.getElementById('report-container').innerHTML = `<p>${translations['invalid_report_data']}</p>`;
            return;
        }
