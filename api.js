...
app.post('/api/save-parent', async (req, res) => {
  try {
    const token = req.headers.authorization?.split(' ')[1];
    const decoded = verifyJWT(token);
    ...
    try {
      ...
      await client.query('COMMIT');
      console.log(`[parent] Parent/Guardian ${guardianIdToLink} saved for participant ${participant_id}`);
      res.json({ success: true, data: { guardian_id: guardianIdToLink }, message: 'Parent saved successfully' });
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    } finally {
