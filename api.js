...
const legacyApiValidation = [check("action").isString().notEmpty()];
...
async function legacyApiHandler(req, res, next) {
  ...
  if (
    !userId &&
    !["login", "register", "request_reset", "reset_password"].includes(action)
  ) {
    ...
    switch (action) {
      ...
      case "get_reunion_preparation":
        ...
        if (reunionPreparationResult.rows.length > 0) {
          ...
          try {
            preparation.youth_of_honor = JSON.parse(
              preparation.youth_of_honor || "[]",
            );
          } catch (error) {
      ...
      case "update_calendar":
        const {
          participant_id: participantIdCal,
          amount,
          amount_paid,
        } = req.body;
        await client.query(
      ...
      case "update_calendar_amount_paid":
        const {
          participant_id: participantIdAmountPaid,
        ...
        jsonResponse(
          ...
          true,
          null,
          "Fundraiser entry amount paid updated successfully",
        );
        break;
      ...
      case "getAvailableDates":
        ...
        jsonResponse(
          res,
          true,
          availableDatesResult.rows.map((row) => row.date),
        );
        break;
      ...
      case "update_attendance":
        const {
          participant_id: participantIdAttendance,
          status,
          date: dateAttendance,
        } = req.body;
        const orgIdForAttendanceUpdate = getCurrentOrganizationId();
        const participantIds = Array.isArray(participantIdAttendance)
          ? participantIdAttendance
          : [participantIdAttendance];
      ...
      case "get_honors":
        ...
        jsonResponse(res, true, {
          ...
          availableDates: availableDatesForHonorsResult.rows.map(
            (row) => row.date,
          ),
        });
        break;
      ...
      case "award_honor":
        ...
        try {
          ...
          const honorPointRules = await getPointSystemRules(
            orgIdForAwardHonor,
            client,
          );
          ...
          for (const honor of honors) {
            ...
            if (honorResult.rows.length > 0) {
              ...
              const groupIdForHonor =
                groupResultForHonor.rows.length > 0
                  ? groupResultForHonor.rows[0].group_id
                  : null;
              ...
              console.log(
                `[honor-legacy] Participant ${participantId} awarded honor on ${date}, points: +${honorAwardPoints}`,
              );
              awards.push({
                participantId,
                awarded: true,
                points: honorAwardPoints,
              });
            } else {
              awards.push({
                participantId,
