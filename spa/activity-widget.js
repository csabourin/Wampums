	...
	async fetchCurrentActivities() {
			try {
					...
					if (data.success && data.preparation && data.preparation.activities) {
							...
							console.log('No activities found in the fetched data');
							this.currentActivities = [];  // Ensure it's set to an empty array
					}
			} catch (error) {
					console.error("Error fetching current activities:", error);
					this.currentActivities = [];  // On error, treat as no activities
			}
	}
