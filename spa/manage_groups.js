import { getGroups, addGroup, removeGroup, updateGroupName } from './ajax-functions.js';
...
export class ManageGroups {
    ...
    renderGroupRows() {
        return this.groups.map(group => `
            <tr>
                <td>
                    <span class="editable-group" contenteditable="true" data-group-id="${group.id}">${group.name}</span>
                </td>
                <td>
                    <button class="remove-group" data-group-id="${group.id}" style="background-color: #f44336;">
    ...
    attachEventListeners() {
        document.getElementById('add-group-form').addEventListener('submit', (e) => this.handleAddGroup(e));
