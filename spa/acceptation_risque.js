...
import { fetchParticipant, fetchAcceptationRisque, saveAcceptationRisque } from './ajax-functions.js';
...
export class AcceptationRisque {
    ...
    render() {
        const content = `
            ...
            <form id="acceptation-risque-form">
                ...
                <p>${translate('nom_participant')}: ${this.participant.first_name} ${this.participant.last_name}</p>
                <p>${translate('age_participant')}: ${this.calculateAge(this.participant.date_naissance)}</p>

                <label for="groupe_district">${translate('groupe_district')}:</label>
                <input type="text" id="groupe_district" name="groupe_district" value="${this.acceptationRisque?.groupe_district || ''}" required>
                ...
                <label for="accepte_risques">
                    <input type="checkbox" id="accepte_risques" name="accepte_risques" ${this.acceptationRisque?.accepte_risques ? 'checked' : ''} required>
                ...
                <p>
                    <label for="participation_volontaire">
                        <input type="checkbox" id="participation_volontaire" name="participation_volontaire" ${this.acceptationRisque?.participation_volontaire ? 'checked' : ''} required>
                        ${translate('participation_volontaire')}
                    </label>
                </p>
                <p>
                    <label for="declaration_sante">
                        <input type="checkbox" id="declaration_sante" name="declaration_sante" ${this.acceptationRisque?.declaration_sante ? 'checked' : ''} required>
                        ${translate('declaration_sante')}
                    </label>
                </p>
                <p>
                    <label for="declaration_voyage">
                        <input type="checkbox" id="declaration_voyage" name="declaration_voyage" ${this.acceptationRisque?.declaration_voyage ? 'checked' : ''} required>
                        ${translate('declaration_voyage')}
                    </label>
                </p>
                <p>
                    <label for="accepte_covid19">
                        <input type="checkbox" id="accepte_covid19" name="accepte_covid19" ${this.acceptationRisque?.accepte_covid19 ? 'checked' : ''} required>
                ...
                <label for="nom_parent_tuteur">${translate('nom_parent_tuteur')}:</label>
                <input type="text" id="nom_parent_tuteur" name="nom_parent_tuteur" value="${this.acceptationRisque?.nom_parent_tuteur || ''}" required>

                <label for="date_signature">${translate('date_signature')}:</label>
                <input type="date" id="date_signature" name="date_signature" value="${this.acceptationRisque?.date_signature || new Date().toISOString().split('T')[0]}" required>

                <input type="submit" value="${translate('soumettre_acceptation_risque')}">
            </form>
    ...
    attachEventListeners() {
        document.getElementById('acceptation-risque-form').addEventListener('submit', (e) => this.handleSubmit(e));
    }
