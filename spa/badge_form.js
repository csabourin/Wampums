...
import { getBadgeProgress, saveBadgeProgress, getCurrentStars } from './ajax-functions.js';
...
export class BadgeForm {
    ...
    render() {
        const content = `
            ...
            <form id="badge-form">
                <label for="territoire_chasse">${translate('territoire_chasse')}:</label>
                ...
                <div id="starInfo">
                    ${translate('current_stars')}: <span id="currentStarsDisplay">0</span>
                </div>

                <input type="hidden" id="currentStars" name="currentStars" value="0">
                <input type="hidden" name="participant_id" value="${this.participant.id}">
                ...
                <label for="date_obtention">${translate('date_obtention')}:</label>
                <input type="date" id="date_obtention" name="date_obtention" required>

                <input type="hidden" id="etoiles" name="etoiles" value="1">

                <input type="submit" id="submitButton" value="${translate('save_badge_progress')}">
            </form>
    ...
    renderBadgeGrid() {
        ...
        return territoires.map(territoire => {
            const badge = this.badgeProgress.find(b => b.territoire_chasse === territoire && b.status === 'approved');
            const stars = badge ? badge.etoiles : 0;
            const pendingStars = this.getPendingStars(territoire);

            return `
                <div class="badge-item">
                    <img src="images/${this.getTerritoireImage(territoire)}" alt="${territoire}">
                    <h3>${territoire}</h3>
                    <div class="stars">
    ...
    renderBadgeDetails(badge) {
        return `
            ...
            <details>
                ...
                <p>${translate('fierte')}: ${badge.fierte ? translate('yes') : translate('no')}</p>
    ...
    getPendingStars(territoire) {
        return this.badgeProgress.filter(b => b.territoire_chasse === territoire && b.status === 'pending').length;
    }
    ...
    async fetchCurrentStars(territoire) {
        try {
            ...
            document.getElementById('currentStarsDisplay').textContent = this.currentStars;
    ...
    updateSubmitButton() {
        ...
        if (this.currentStars >= 3 || this.hasPending) {
            ...
            submitButton.value = this.currentStars >= 3 ? translate('max_stars_reached') : translate('pending_submission_exists');
        } else {
        ...
        if (this.validateForm(form)) {
            try {
                ...
                if (result.status === 'success') {
                    this.showSuccessMessage(translate('badge_progress_submitted_for_approval'));
                    await this.fetchBadgeProgress();
                    this.render();
                } else {
    ...
    validateForm(form) {
        ...
        form.querySelectorAll('[required]').forEach(field => {
            if (!field.value.trim()) {
                isValid = false;
                errorMessages.push(`${field.previousElementSibling.textContent.replace(':', '')} is required.`);
            }
        });
