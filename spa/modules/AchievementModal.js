...
export class AchievementModal {
    ...
    renderBadgeGrid() {
        ...
        const badgeCards = this.badgeTemplates.map(badge => {
            const isSelected = String(badge.id) === String(this.selectedBadgeId);
            const imageUrl = badge.image || badge.image_url || '';
            const badgeName = translate(badge.translation_key) || badge.name || translate("badge_unknown_label");
            ...
            return `
                                <button type="button" 
                                        ...
                                        ${imageUrl
                    ? `<img src="${escapeHTML(imageUrl)}" alt="${escapeHTML(badgeName)}" class="achievement-badge-card__image" />`
    ...
    renderParticipantGrid() {
        ...
        const participantCheckboxes = sortedParticipants.map(p => {
            const isChecked = this.selectedParticipantIds.includes(String(p.id)) || this.selectedParticipantIds.includes(p.id);
    ...
    attachListeners() {
        ...
        modal.querySelectorAll('.achievement-type-btn').forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                modal.querySelectorAll('.achievement-type-btn').forEach(b => b.classList.remove('active'));
                e.target.classList.add('active');

                const participantsSection = modal.querySelector('.achievement-modal__participants');
                if (participantsSection) {
                    participantsSection.classList.toggle('hidden', this.selectedType === 'battue');
                }
            });
        });
        ...
        modal.querySelectorAll('.achievement-badge-card').forEach(card => {
            card.addEventListener('click', (e) => {
                ...
                modal.querySelectorAll('.achievement-badge-card').forEach(c => c.classList.remove('selected'));
    ...
    handleSave() {
        ...
        const modal = document.getElementById(this.modalId);
        const checkedBoxes = modal?.querySelectorAll('.achievement-participant-checkbox:checked') || [];
        this.selectedParticipantIds = Array.from(checkedBoxes).map(cb => cb.value);
