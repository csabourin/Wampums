 ...
 * Handles quick activity creation in a modal for permission slips
 */
class QuickCreateActivityModal {
      ...
      <div style="background: white; border-radius: 12px; max-width: 500px; width: 90%; padding: 2rem;">
        ...
        <form id="quick-create-activity-form">
          ...
          <div style="margin: 1.5rem 0; padding-top: 1.5rem; border-top: 2px solid #e9ecef;">
            <h3 style="margin: 0 0 1rem 0; font-size: 1.1rem; color: #28a745;">${translate("returning_from_activity")}</h3>
  ...
  async handleSubmit(e) {
    ...
    try {
      ...
      // Redirect to permission slip page for this activity
      this.app.setTimeout?.(() => {
        window.location.hash = `/permission-slips/${newActivity.id}`;
      }, 500);
    } catch (error) {
