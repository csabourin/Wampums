  ...
  form.addEventListener("submit", function (e) {
    ...
    form.querySelectorAll("[required]").forEach((field) => {
      if (!field.value.trim()) {
        isValid = false;
        errorMessages.push(
          `Le champ "${field.previousElementSibling.textContent.replace(
            ":",
            ""
          )}" est requis.`
        );
    ...
    ["contact_urgence_1_telephone", "contact_urgence_2_telephone"].forEach(
      (id) => {
        const phoneField = document.getElementById(id);
        if (phoneField.value && !isValidPhone(phoneField.value)) {
          isValid = false;
          errorMessages.push(
            `Le numéro de téléphone "${phoneField.previousElementSibling.textContent.replace(
              ":",
              ""
            )}" n'est pas valide.`
          );
    ...
    if (reglesCheckbox.checked && !renseigneeCheckbox.checked) {
      isValid = false;
      errorMessages.push(
        "Veuillez confirmer que la jeune femme est renseignée sur les règles."
      );
    }
