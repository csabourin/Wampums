...
CREATE TABLE public.permission_slips (
  ...
  meeting_id integer,
  meeting_date date NOT NULL,
  status character varying DEFAULT 'pending'::character varying CHECK (status::text = ANY (ARRAY['pending'::character varying, 'signed'::character varying, 'declined'::character varying, 'revoked'::character varying, 'expired'::character varying, 'archived'::character varying]::text[])),
  ...
  CONSTRAINT permission_slips_activity_id_fkey FOREIGN KEY (activity_id) REFERENCES public.activities(id)
);
...
CREATE TABLE public.ai_usage_monthly (
  ...
  updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP,
  CONSTRAINT ai_usage_monthly_pkey PRIMARY KEY (month_key)
);
